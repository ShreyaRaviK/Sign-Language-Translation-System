...
# Pose indices for 11 upper-body joints (MediaPipe Pose)
POSE_IDX = [0, 11, 12, 13, 14, 15, 16, 23, 24, 25, 26]
POSE_IDX_LEFT_HIP = POSE_IDX.index(23)
POSE_IDX_RIGHT_HIP = POSE_IDX.index(24)
POSE_IDX_LEFT_SHOULDER = POSE_IDX.index(11)
POSE_IDX_RIGHT_SHOULDER = POSE_IDX.index(12)


def build_nodes(pose_arr, hand_arr):
    # pose_arr: (T, 33, 4), hand_arr: (T, 2, 21, 4)
    pose_sel = pose_arr[:, POSE_IDX, :]
    hands = hand_arr.reshape(hand_arr.shape[0], -1, hand_arr.shape[-1])
    return np.concatenate([pose_sel, hands], axis=1)


def normalize_and_velocity(nodes):
    # nodes: (T, N, 4) -> returns (T, N, 7): [x,y,z,vis,dx,dy,dz]
    nodes = nodes.astype(np.float32, copy=False)
    coords = nodes[..., :3]
    vis = nodes[..., 3:4]

    # Use pose joints (first 11) to compute a stable center/scale.
    pose = coords[:, :len(POSE_IDX), :]
    pose_vis = vis[:, :len(POSE_IDX), 0]

    pose_mask = (pose_vis > 0).astype(np.float32)
    denom = pose_mask.sum(axis=1, keepdims=True)
    denom = np.where(denom == 0, 1.0, denom)
    pose_mean = (pose * pose_mask[..., None]).sum(axis=1, keepdims=True) / denom[..., None]

    hip_center = (pose[:, POSE_IDX_LEFT_HIP] + pose[:, POSE_IDX_RIGHT_HIP]) / 2.0
    hip_center = hip_center[:, None, :]
    hip_visible = (pose_vis[:, POSE_IDX_LEFT_HIP] > 0) & (pose_vis[:, POSE_IDX_RIGHT_HIP] > 0)

    center = pose_mean.copy()
    center[hip_visible] = hip_center[hip_visible]

    shoulder_vec = pose[:, POSE_IDX_LEFT_SHOULDER] - pose[:, POSE_IDX_RIGHT_SHOULDER]
    shoulder_dist = np.linalg.norm(shoulder_vec, axis=-1, keepdims=True)
    shoulder_visible = (pose_vis[:, POSE_IDX_LEFT_SHOULDER] > 0) & (pose_vis[:, POSE_IDX_RIGHT_SHOULDER] > 0)
    scale = np.where(shoulder_visible[:, None], shoulder_dist, 1.0)
    scale = np.where(scale < 1e-6, 1.0, scale)

    norm_coords = (coords - center) / scale[:, None, :]

    vel = np.zeros_like(norm_coords)
    vel[1:] = norm_coords[1:] - norm_coords[:-1]

    return np.concatenate([norm_coords, vis, vel], axis=-1)
